...
import { compileStyle, compileStyleAsync } from '@vue/component-compiler-utils';
import chalk from 'chalk';
import fs from 'fs';
import swcCore from '@swc/core';
import { SourceNode, SourceMapConsumer, SourceMapGenerator } from '@cspotcode/source-map';
import { SourceNode as SourceNode$1, SourceMapConsumer as SourceMapConsumer$1, SourceMapGenerator as SourceMapGenerator$1 } from 'source-map';
        ...
        if (result.map) {
            node.add(
                SourceNode$1.fromStringWithSourceMap(
                    result.code,
                    await new SourceMapConsumer$1(result.map)
                )
            );
        } else {
            node.add(result.code);
...
async function mapLinesAsync(oldMap, newMap) {
  ...
  const oldMapConsumer = await new SourceMapConsumer$1(oldMap);
  const newMapConsumer = await new SourceMapConsumer$1(newMap);
  const mergedMapGenerator = new SourceMapGenerator$1();
